...
import com.foxconn.iisd.bd.rca.XWJKernelEngine.{configContext, configLoader, job}
...
class TestDetailDataProcessing(configContext: ConfigContext) extends BaseDataProcessing {
  ...
  def  calculation (): Unit  = {
    ...
    if (snSize > 0) {
      ...
      //1-1: 取得所有product的最早跟最新test_start_time
      val productTestStartTimeTemp = testDetailCockroachDf.groupBy(col("product"))
      val productTestStartTimeMin = productTestStartTimeTemp
      .agg(min("test_starttime").as("test_starttime"))
      .select($"product", $"test_starttime").as[(String, String)].collect.toMap.asJava

      println("======> testDetailFirstRowTestStartTimeByProduct : " + productTestStartTimeMin)
      SummaryFile.testDetailFirstRowTestStartTimeByProductInCockroachdb = productTestStartTimeMin

      val productTestStartTimeMax = productTestStartTimeTemp
        .agg(max("test_starttime").as("test_starttime"))
        .select($"product", $"test_starttime").as[(String, String)].collect.toMap.asJava

      println("======> testDetailLastRowTestStartTimeByProduct : " + productTestStartTimeMax)
      SummaryFile.testDetailLastRowTestStartTimeByProductInCockroachdb = productTestStartTimeMax
      ...
      var productStationDf = configContext.mysqlDBIo.getDfFromMariadbWithQuery(configContext.sparkSession,

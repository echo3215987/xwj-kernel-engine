CREATE DATABASE  IF NOT EXISTS `rca-cartridge-nesta` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `rca-cartridge-nesta`;
-- MySQL dump 10.13  Distrib 5.7.24, for Linux (x86_64)
--
-- Host: 10.57.232.61    Database: rca-cartridge-nesta
-- ------------------------------------------------------
...
--
-- Dumping routines for database 'rca-cartridge-nesta'
--
...
create
	temporary table
		fill_pfl_tmp select
			...
			line
		FROM
			`rca-cartridge-nesta`.risk_assembly_station;
...
BEGIN
  ...
  select *
  FROM `rca-cartridge-nesta`.risk_assembly_station where product=pdt and floor=flr and test_starttime >= starttime and test_starttime < stoptime;
  ...
  create temporary table level_one_tmp
  select *
  FROM `rca-cartridge-nesta`.risk_assembly_station where product=pdt and test_starttime >= starttime and test_starttime < stoptime;
...
BEGIN
  ...
  FROM `rca-cartridge-nesta`.risk_assembly_station where product=pdt and floor=flr and test_starttime >= starttime and test_starttime < stoptime group by riskcode;
  else
   create temporary table level_one_tmp
  select riskCode,riskname,failQty,defectQty,
  round(sum(failQty)/sum(defectQty),10) as commonnality,outputQty,throughputQty,
  ...
  sum(throughputQty),
  round((sum(risk_failureduration_sec)/ sum(asm_duration_sec))*100,2) as failureContRation 
  FROM `rca-cartridge-nesta`.risk_assembly_station where product=pdt and test_starttime >= starttime and test_starttime < stoptime group by riskcode;
  end if;
SELECT 
    MAX(commonnality)
...
BEGIN
  ...
  sum(throughputQty),
  round((sum(risk_failureduration_sec)/ sum(asm_duration_sec))*100,2) as failureContRation 
  FROM `rca-cartridge-nesta`.risk_test_station where product=pdt and floor=flr and test_starttime >= starttime and test_starttime < stoptime group by riskname;
  else
   create temporary table level_two_tmp
  select product,factory,floor,symptonid,failure_sympton,riskname,riskcode,failQty,defectQty,
  round(sum(failQty)/sum(defectQty),10) as commonnality,outputQty,throughputQty,
  ...
  sum(throughputQty),
  round((sum(risk_failureduration_sec)/ sum(asm_duration_sec))*100,2) as failureContRation 
  FROM `rca-cartridge-nesta`.risk_test_station where product=pdt and test_starttime >= starttime and test_starttime < stoptime group by riskname;
  end if;
SELECT 
    MAX(commonnality)
INTO @max_commonnality FROM
...
WHEN 'test' THEN create
	temporary table
		level_three_tmp select
			...
			round((sum(risk_failureduration_sec)/ sum(asm_duration_sec))* 100, 2) as failureContRation
		FROM
			`rca-cartridge-nesta`.risk_test_station
		where
			product = pdt
			and floor = flr
			and teststation = tstation
